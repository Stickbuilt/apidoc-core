    ...
    for (var i = 0; i < indexApiBlocks.length; i += 1) {
        ...
        for (var j = 0; j < elements.length; j += 1) {
            ...
            if ( ! elementParser) {
                ...
                if (element.sourceName === 'apiignoreall') {
                    continue;
                }
 ...
 *
 * An @apiIgnoreAll ignores the block.
...
Parser.prototype._findBlockWithApiGetIndex = function(blocks) {
    ...
    for (var i = 0; i < blocks.length; i += 1) {
        ...
        for (var j = 0; j < blocks[i].length; j += 1) {
            if (blocks[i][j].name.substr(0, 12) === 'apiignoreall') {
                app.log.debug('apiIgnoreall found in block: ' + i);
                found = false;
                break;
            }
