...
Parser.prototype._findBlockWithApiGetIndex = function(blocks) {
    ...
    for (var i = 0; i < blocks.length; i += 1) {
        ...
        for (var j = 0; j < blocks[i].length; j += 1) {
            ...
            if (blocks[i][j].name.substr(0, 11) === 'apiinternal') {
                app.log.debug('apiInternal found in block: ' + i);
                if (app.options.external == true) {
                    app.log.debug("external is true")
                    found = false;
                    break;
                } else {
                    app.log.debug("external is false")
                    continue;                    
                }
            }
